...
public class p_WriteResults
{
    ...
    private static void WriteSearchHit(XMLStreamWriter writer, SearchHit sh, String OldPepSeq, String NewPepSeq, String NewDeltaMass, String NewPepMass, String peptide_prev_aa, String peptide_next_aa, String ProtId, String AddNumMissedCleavage) throws XMLStreamException
    {
        try{
            ...
            //PTM result
            // do not write PTM localization result if the peptide sequence was changed and that removes the delta mass
            boolean skipWritingPTM = false;
            if (!OldPepSeq.equals(NewPepSeq)){
                double newDeltaMass = Double.parseDouble(NewDeltaMass);
                if (newDeltaMass > -1.5 && newDeltaMass < 3.5) {
                    skipWritingPTM = true;
                }
            }
            if (!skipWritingPTM) {
